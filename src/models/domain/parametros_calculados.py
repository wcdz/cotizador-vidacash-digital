...
class ParametrosCalculados:
    ...
    def calcular_inflacion_mensual(inflacion_anual: float) -> float:
    ...
    def calcular_tasa_interes_anual(
        tasas_interes_data: dict, periodo_vigencia: int
    ) -> float:
    ...
    def calcular_tasa_inversion(
        tasas_interes_data: dict, periodo_pago_primas: int
    ) -> float:
    ...
    def calcular_tasa_costo_capital_mes(tasa_costo_capital_tir: float) -> float:
        """Calcula la tasa de costo capital mensual"""
        return (1 + tasa_costo_capital_tir) ** (1 / 12) - 1

    def calcular_factor_pago(
        frecuencia_pago_primas: FrecuenciaPago, factores_pago: dict
    ) -> float:
        """
    ...
    def calcular_tasa_frecuencia_seleccionada(
        prima_para_redondeo: float, suma_asegurada: float
    ) -> float:
        """Calcula la tasa de frecuencia seleccionada"""
        return prima_para_redondeo / suma_asegurada

    def calcular_factor_ajuste(producto: Producto) -> float:
