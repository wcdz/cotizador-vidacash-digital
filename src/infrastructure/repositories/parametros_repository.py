...
class JsonParametrosRepository(ParametrosRepository):
    ...
    def _get_parametros_path(self, producto: str, cobertura: str = None) -> Path:
        """Obtiene la ruta al archivo de parámetros para un producto y opcionalmente una cobertura"""
        if cobertura:
            # Si base_path ya contiene la ruta completa (viene de repos.py), usarla directamente
            if "coberturas" in str(self.base_path):
                return self.base_path / "parametros.json"
            else:
                # Ruta para coberturas: assets/productos/endosos/coberturas/fallecimiento/parametros.json
                return self.base_path / "productos" / producto.lower() / "coberturas" / cobertura.lower() / "parametros.json"
        else:
            # Si base_path ya contiene la ruta completa (viene de repos.py), usarla directamente
            if "productos" in str(self.base_path) or "endosos" in str(self.base_path):
                return self.base_path / "parametros.json"
            else:
                # Ruta normal para producto: assets/endosos/parametros.json
    ...
    def _cargar_parametros_cobertura(self, producto: str, cobertura: str) -> Dict[str, Any]:
        """
        Carga los parámetros desde el archivo JSON para una cobertura específica
        Simula una consulta a la base de datos
        """
        ...
        parametros_path = self._get_parametros_path(producto, cobertura)
        ...
        try:
            with open(parametros_path, "r", encoding="utf-8") as f:
                parametros = json.load(f)
                self._cache[cache_key] = parametros
                return parametros
        except (json.JSONDecodeError, IOError) as e:
            print(f"Error cargando {producto}/{cobertura}: {e}")
            self._cache[cache_key] = {}
            return {}
    
    def get_parametros_by_producto_and_cobertura(self, producto: str, cobertura: str) -> Dict[str, Any]:
        """
        Obtiene todos los parámetros para un producto y una cobertura
        Simula un SELECT * FROM parametros WHERE producto = ? AND cobertura = ?
        """
        return self._cargar_parametros_cobertura(producto, cobertura)
