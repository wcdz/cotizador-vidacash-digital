...
class JsonCaducidadRepository(CaducidadRepository):
    ...
    def __init__(self, base_path: str = None, producto: str = "endosos", coberturas: Optional[List[str]] = None):
        """
        Inicializa el repositorio de caducidad
        
        Args:
            base_path: Ruta base para los archivos JSON (optional)
            producto: Nombre del producto (default: "endosos")
            coberturas: Lista de coberturas específicas (optional)
        """
        if base_path:
            self.base_path = Path(base_path)
        else:
            # Determinar ruta según si hay coberturas específicas
            if coberturas and len(coberturas) > 0:
                # Con coberturas específicas: assets/productos/{producto}/coberturas/{cobertura}/
                primera_cobertura = coberturas[0].lower()
                self.base_path = (Path(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))) / 
                                "assets" / "productos" / producto.lower() / "coberturas" / primera_cobertura)
            else:
                # Sin coberturas: ruta genérica assets/{producto}/
